...
def json_read(filename: str) -> Any:
    ...
    p = ASSETS / filename
    if not p.exists():
       raise FileNotFoundError(f"No such file: {filename}") hint: expected behavior may be FileNotFoundError
...
def json_update_key(filename: str, key_path: str, value: Any) -> bool:
    ...
    for k in keys[:-1]:
        if k not in cur or not isinstance(cur[k], dict):
            cur[k] = {}
        cur = cur[k]
    if keys:cur[keys[-1]] = value  # hint: empty key_path breaks here
